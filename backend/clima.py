...
class Clima:
    ...
    def actualizar_clima(self):
        if self.transitioning:
            self.paso_actual += 1
            if self.paso_actual >= self.pasos_total:
                self.transitioning = False
                self._iniciar_burst(self.pending_burst)
            else:
                interpolado = self.m_inicial + (self.m_final - self.m_inicial) * (self.paso_actual / self.pasos_total)
    ...
    def _transicion_suave(self, nuevo_burst):
        self.m_inicial = self.multiplicador_actual
        self.m_final = self.MULTIPLICADORES.get(nuevo_burst["condition"], 1.0)
        self.m_final = round(self.m_final * (1 - 0.2 * nuevo_burst["intensity"]), 3)

        print(f" Transición suave: {self.clima_actual} → {nuevo_burst['condition']}")
        print(f" Multiplicador: {self.m_inicial} → {self.m_final}")

        self.transitioning = True
        self.paso_actual = 0
        self.pending_burst = nuevo_burst
